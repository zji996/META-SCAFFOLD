...
- 新增产品界面信息层级：可见文字必须承担对象、状态、动作、后果、错误或必要指引之一；否则默认删除。
- 明确表达顺序为简化结构与领域名词、熟悉图标、必要短文案，并保留 tooltip、可访问名称及真正影响行为的权限/模式提示。
- 空态默认使用一个图标加一句可执行提示，避免 eyebrow、标题、副标题和说明段重复解释同一件事。
...
- 弱化「全仓统一端口偏移」（如 `port_instance=N` → 全体 `Nxxx`）作为治理主轴；upstream / 基础设施端口只要本机不冲突即可。
- 重排多实例端口优先序：域名入口 → 动态端口 → 仅隧道/回调等必须预知端口时才用实例前缀；`ports`/`urls`/`status` 区分约定入口与真实监听，停干净后不得把域名表展示成已可访问。

## 6.12.1

- 调整设计沙盒标识策略：design/mock 身份由独立入口、页面元数据和快照说明表达，默认不在评审与截图画布上叠加工具栏、水印或切换器。
- 保留“设计稿不能冒充已实现功能”的证据边界，同时避免治理 chrome 污染视觉评审基线。

## 6.12.0

- 新增设计沙盒与生产路径隔离：高频 UI 探索使用独立开发态 app/build，不在生产 App 留查询参数、认证旁路或假业务状态。
- 规定 sandbox 只含 mock 场景与有限假交互，不接生产 API、不与生产业务模块形成双向依赖，也不进入生产导航、镜像、静态托管或发布构建。
- 定稿采用截图加场景说明，区分视觉基线、设计稿可运行证据和生产功能完成证据；补充多画布、独立生命周期与共享边界判断模式。
