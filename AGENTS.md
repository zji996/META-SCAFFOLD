...
- 保持 `skills/meta-scaffold/` 作为唯一运行时内容源，核心规则在 `SKILL.md`，细节在 `references/`。
- `prompts/META-SCAFFOLD-v6.md` 是人工审阅版，不反向覆盖 runtime skill。
- 所有分发件（dist / templates / plugin / cursor rule）保持为薄适配器，不各自长篇维护。
