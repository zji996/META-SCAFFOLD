...
- 以 v6.12.1 作为 Pi/Codex/Kilo/Cursor 共用的目的驱动单 runtime 基线：Pi 委派使用前台 JSON 流，本地门禁优先于新增托管 CI，验证后的边界清晰改动默认创建原子本地 commit；多项目开发优先动态端口自省与单一系统入口；快速 UI 探索与生产交付使用隔离且不污染截图画布的证据路径。
