...
- 以 v6.15.0 作为 Pi/Codex/Kilo/Cursor 共用的目的驱动单 runtime 基线：同机浏览器使用 Caddy 域名入口，远程进程机通过公网 Caddy + FRP 发布静态 edge；界面先简化结构、再用熟悉图标、最后才补必要短文案；其余继续保持前台 Pi JSON 流、本地门禁、原子本地 commit 与设计/生产证据隔离。
