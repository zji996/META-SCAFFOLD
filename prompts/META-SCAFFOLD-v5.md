...
v5 保留 v4 的核心判断：现实优先、文档最小化、monorepo 不强制、`apps/` 放运行单元、`docs/current.md` 压缩上下文、命令入口统一、`scaffold.plan.yaml` 可选。

v5 新增或强化：

- 增加 `Frame` 与 `Compact`，让目标和交接更稳定。
- 增加“先想清楚、简洁优先、精准修改、目标驱动”的行为原则。
- 增加风险分级，减少不必要提问，同时拦截高风险改动。
- 增加上下文预算分级，避免每轮读全仓库。
- 增加 skill 化组织方式，支持作为单文件或可复用 skill 使用。
- 增加新项目初始化协议和已有项目治理协议。
- 增加输出格式和完成质量自检。
