...
这不是死板流程，是安全边界。简单任务可压缩表达，但不能跳过关键判断。
...
现代 agent 拥有搜索、读取、编辑、执行、子任务等多种工具。高效协作要求工具使用有纪律，而非随手调用。

### 3.1 批量并行，不串行浪费

多个**相互独立**的读取/搜索应一次性并行发起，不要逐个等待。典型：同时读 AGENTS、current.md、相关配置文件；同时跑 glob + grep 定位代码。

例外：后一步依赖前一步结果时（如先读完才知道改哪里），必须串行。

### 3.2 先读后改

编辑文件前先读它。不读就改等于盲改。对未读文件不做 `replaceAll` 等大范围操作。
...
- 不知路径、范围小 → Glob 找文件名。
- 不知内容位置 → Grep 找内容。
- 开放式、需多轮探索 → 用 subagent（见第 4 章）。

### 3.4 执行纪律

- 用专用工具而非 shell 做文件操作（Read 而非 `cat`，Edit 而非 `sed`）。
- 开发服务器、watcher、长进程用进程管理工具，不用 `&`/`nohup`/`disown` 后台化。
- 危险或不可逆命令（`rm -rf`、`git push --force`、迁移）默认不执行，除非用户明确授权。
- 命令失败如实报告，不自动重试到"看起来成功"。
...
规则：用户要求继续 goal/推进 plan 时，先读顶部 ledger，从第一个未勾选项继续；checklist 项要小到一轮可验证；每轮结束/压缩/换目标/遇 blocker 前更新 checkbox、`Next unchecked item`、blocker；稳定事实写入 current/roadmap/reference，不要只留在可能被忽略的 plan 里；不在 plan 写 secrets。
...
| Kilo 工具 | 对应契约原则 | 触发条件 |
| --- | --- | --- |
| `agent_manager`（worktree/local 会话） | 第 4 章并行编排 | 满足 4.1 全部条件，且需要**写隔离**或多版本对比（`versions=true`）。只读研究用 subagent/Task 即可，无需 worktree |
| `task`（内部子 agent） | 第 4 章 / 第 3.3 搜索分层 | 开放式多轮探索、独立研究、独立模块实现。给自包含指令并说明返回什么 |
| `background_process` | 第 3.4 执行纪律 | 开发服务器、watcher、长进程（`npm run dev`、`vite`、`pytest --watch`）。不用 `&`/`nohup` |
| `suggest`（code review 建议） | 第 2.7 Handoff | 完成非平凡改文件工作后，针对未提交工作树建议 `/local-review-uncommitted`；已提交用 `/local-review`。琐碎改动、修 review、已建议过时不触发 |
| `question` | 第 2.3 Decide | 高影响/不可逆歧义才问，低风险用安全默认值 |
| 批量 Read/Glob/Grep | 第 3.1 批量并行 | 多个独立读取/搜索一次性并行 |

适配原则（重申第 4.4）：`agent_manager`/`task` 的使用是**条件触发**，不强求。Kilo 模型若不擅长并行分发，回退串行。worktree 仅用于真正需要写隔离的并行；纯研究用 `task` 足够。
