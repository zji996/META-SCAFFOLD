...
需要把子任务交给其他 agent CLI（如 Codex 调 Grok）时：只走 headless（`grok -p "..."` + `--yolo`），写清任务边界；默认不互调、不嵌套、不并行改同一工作树；结束后主控自行 diff 与验证。命令示例见 [references/platforms.md](references/platforms.md)。
...
| `.local/plan/` | 可恢复本地账本；不存稳定事实或 secrets |
