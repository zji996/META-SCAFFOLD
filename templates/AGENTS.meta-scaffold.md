...
如果改动会影响后续工作，只把和判断相关的信息写入 `docs/current.md`：当前焦点、短期下一步、阻塞、关键架构事实、验证命令。**只记短期当前状态**——已完成 goal 不在此留存细节（压缩成一行指针指向 roadmap 或状态表），修复历史归 `git log`。目的是让下一轮 AI 用最少 token 接上当前工作。方向性决策写 `docs/decision/00NN-*.md`（ADR）。
